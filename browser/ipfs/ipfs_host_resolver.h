...
class IPFSHostResolver : public network::ResolveHostClientBase {
 public:
  ...
  void SetResetCallbackForTesting(base::OnceClosure reset_callback) {
    reset_callback_for_testing_ = std::move(reset_callback);
  }
 ...
 private:
  ...
  base::OnceClosure reset_callback_for_testing_;
